...
function transferColumns(config, importParameters, factory, pageType, routingId, configId, viewId) {
    ...
    if (lineItems && Object.keys(lineItems.properties).length > 0) {
        ...
        const views = importParameters.manifest['sap.ui5']?.routing?.targets?.[routingId]?.options?.settings?.views;
    ...
    for (const element of sortedList) {
        ...
        if (element[1]['$ref']) {
            ...
            if (!schemaDefinitions[nextDefinition].entitySet) {
                (0, utils_1.importSettingsOfObject)(tableView, importParameters.manifest, schemaDefinitions[nextDefinition], routingId, [element[0], targetAnnotation]);
